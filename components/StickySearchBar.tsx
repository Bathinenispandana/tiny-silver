...
export function StickySearchBar() {
  ...
  return (
    <div className="fixed top-0 left-0 right-0 z-40 bg-background border-b border-primary/20">
      <div className="max-w-7xl mx-auto px-4 py-4">
        {/* Header Row */}
        <div className="flex items-center justify-between gap-6">
          {/* Logo */}
          <Link href="/" className="flex-shrink-0">
            <h1 className="text-2xl font-bold text-primary">LUXE</h1>
          </Link>
          ...
          <div className="flex-shrink-0 relative">
            ...
            >
              ...
              <div className="text-left">
                <p className="text-xs text-primary/60 font-medium">Where to Deliver?</p>
                <p className="text-sm font-semibold text-primary">Update Delivery Pincode</p>
              </div>
            </button>
            ...
            {showPincodeDropdown && (
              <div className="absolute top-full mt-2 left-0 bg-background border border-primary/30 rounded-lg shadow-lg p-3 min-w-48 z-50">
                <input
                  ...
                  className="w-full px-3 py-2 border border-primary/20 rounded-lg text-primary placeholder:text-primary/40 outline-none focus:border-primary/60 bg-primary/5 text-sm"
                />
                <button
                  onClick={() => {
                    setShowPincodeDropdown(false)
                  }}
                  className="w-full mt-2 px-3 py-2 bg-primary text-background rounded-lg font-medium hover:bg-primary/90 transition-colors text-sm"
                >
                  Apply
                </button>
                <p className="text-xs text-primary/50 mt-2">Current: {pincode}</p>
              </div>
            )}
          </div>
          ...
          <form onSubmit={handleSearch} className="flex-1 max-w-2xl flex items-center gap-3 bg-primary/5 border border-primary/30 rounded-lg px-4 py-3 hover:border-primary/60 transition-all">
            <Search size={20} className="text-primary/60 flex-shrink-0" />
            <input
              type="text"
              placeholder={`Search "Pendants"`}
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="flex-1 bg-transparent text-primary placeholder:text-primary/50 outline-none text-sm"
            />
            {searchQuery && (
              <button
                type="button"
          ...
          <div className="flex-shrink-0 flex items-end gap-6">
            ...
            <Link
              href="/stores"
              className="flex flex-col items-center gap-1 text-primary hover:text-accent transition-colors group"
            >
            ...
            <Link
              href="/wishlist"
              className="flex flex-col items-center gap-1 text-primary hover:text-accent transition-colors group relative"
            >
